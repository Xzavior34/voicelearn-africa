...
1. Learner presses the mic and speaks (or types, if speech recognition is unavailable — see
   below).
2. Every incoming transcript is first classified against the *current* conversation state by
   `lib/tutor/session.ts`'s `classifyRelation` as one of `new_topic`, `continuation`,
   `answer_to_previous`, or `ambiguous` — this is what stops an unrelated new question from ever
   being scored as a wrong answer to whatever the learner asked previously, or answered with a
   stale explanation left over from an earlier topic. Curriculum matching *improves* this
   classification when available but is never required to detect a new topic — a question the
   curriculum has never heard of is still correctly recognized as new, not silently answered from
   the old topic.
3. `new_topic`/first turns go through `lib/tutor/intent.ts` (educational intent + topic
   extraction) and `lib/tutor/reasoning.ts` (a structured, schema-validated explanation and
   follow-up question from the curriculum in `lib/tutor/curriculum.ts`).
4. `answer_to_previous`/`continuation` turns go through `lib/tutor/assessment.ts`, which evaluates
   the answer against the concept currently active (not exact-text matching — it normalizes
   numerals, detects known misconceptions, and gives partial credit).
5. `lib/tutor/adaptation.ts` updates difficulty and session state; the next question is picked
   accordingly. A genuine topic switch resets difficulty/attempts/misconceptions rather than
   carrying the old topic's progress into the new one.
6. On the client, `lib/client/turnGuard.ts` gives every turn a unique id bundled with an
   `AbortController`: starting a new turn aborts the previous request outright, and a response
   that arrives after a newer turn has already started is discarded rather than overwriting
   state — this also covers duplicate submissions and rapid repeated mic taps.
...
Microphone → /api/speech (Sahara provider) → transcript
  → /api/tutor → relation classifier → intent → reasoning → assessment → adaptation → next question
```
...
- **Mobile Microphone**: `MediaRecorder` audio capture is implemented and unit-tested; end-to-end verification on a physical smartphone browser requires a local device test (`LOCAL_DEVICE_TEST_REQUIRED` — see `ANDROID_TEST_CHECKLIST.md` / `PHONE_TEST_SCRIPT.md`).
...
- **Curriculum Scope**: The curriculum covers 9 secondary-school concepts across mathematics, science, and English (signed multiplication, division by zero, photosynthesis/chlorophyll, evaporation, friction/rolling motion, dissolving, main idea identification, affect vs. effect, nouns) — intentionally focused, not a general-purpose subject model.
...
- **Conversation relation classifier**: `new_topic`/`continuation`/`answer_to_previous`/`ambiguous` classification is a deterministic, rule-based heuristic (question structure, anaphoric reference, explicit topic-abandon phrasing, curriculum matching where available) — not a semantic/embedding-based classifier. It is covered by a 34-case golden corpus but can still misclassify phrasing outside that corpus's patterns.
...
- Continue expanding the curriculum with real-learner feedback; replace the rule-based relation classifier with a semantic/embedding-based one if misclassifications are observed in real use.
