...
| **3. Real Sahara audio transcription** | `lib/speech/providers/sahara.ts`, `lib/benchmark/runner.ts` | **VERIFIED** | 2 physical audio recordings streamed and transcribed live via Sahara, including one genuine code-switched recording. See `FINAL_SUBMISSION_EVIDENCE.md`. |
...
| **5. Four-model benchmark** | `lib/benchmark/runner.ts`, `lib/speech/registry.ts`, `scripts/run-benchmark.ts` | **VERIFIED** | Compares Sahara, OpenAI Whisper Tiny, OpenAI Whisper Base, and Meta Wav2Vec2 Base 960h against identical evaluation audio and ground-truth transcripts. All four `VERIFIED` (`benchmark/results/summary.json`). |
| **6. Real WER/CER** | `lib/benchmark/metrics.ts`, `lib/benchmark/runner.ts` | **VERIFIED** | Levenshtein WER/CER algorithms verified by unit tests (`__tests__/metrics.test.ts`) and measured for real on physical audio: Sahara 40.2%/37.0%, Whisper Tiny 25.0%/11.9%, Whisper Base 17.9%/10.3%, Wav2Vec2 Base 960h 71.4%/34.7% (see `FINAL_SUBMISSION_EVIDENCE.md`). |
| **7. Intent accuracy** | `lib/tutor/intent.ts`, `lib/benchmark/reports/intent-baseline-latest.json` | **VERIFIED** | **78.8%** accuracy (26/33 initial-question samples) across secondary math, science, and English curriculum on ground-truth transcripts. |
