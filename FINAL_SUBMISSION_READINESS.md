...
> **⚠️ SUPERSEDED (2026-09-14):** the WER/CER figures in this file (3.6%/0.8%, or elsewhere 7.1%/6.3%) predate a real, current four-model benchmark run and predate the `vl-035` code-switched recording existing at all. **See `FINAL_SUBMISSION_EVIDENCE.md` for the current state**: Sahara, Whisper Tiny, Whisper Base, and Wav2Vec2 Base 960h are all `VERIFIED`, measured on 2 physical recordings (one genuinely code-switched), and phone/device verification referenced below as pending has since happened. This file is kept as a historical audit record — do not cite its numbers in the final submission.
## STATUS
